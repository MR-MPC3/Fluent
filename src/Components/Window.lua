...
return function(Config)
	...
	local ButtonIsDragged = false
	local ButtonAnimationId = 0
	...
	local function AnimateButton(ScaleValue, Time, Style)
		StopButtonTween()
	...
	local function BounceButton()
		ButtonAnimationId = ButtonAnimationId + 1
	
		local AnimationId = ButtonAnimationId
	
		AnimateButton(0.82, 0.08)
	
		task.delay(0.08, function()
			if AnimationId ~= ButtonAnimationId or ButtonDragging then
				return
			end
	
			AnimateButton(1.08, 0.16, Enum.EasingStyle.Back)
			...
			task.delay(0.16, function()
				...
				AnimateButton(1, 0.12)
			end)
		end)
	end
	
	Creator.AddSignal(FatCatButton.InputBegan, function(Input)
		if
		...
		ButtonStartPos = FatCatButton.Position
	
		ButtonAnimationId = ButtonAnimationId + 1
	
		AnimateButton(0.88, 0.12)
	end)
	...
	Creator.AddSignal(UserInputService.InputEnded, function(Input)
		...
		AnimateButton(1.08, 0.18, Enum.EasingStyle.Back)
	...
	Creator.AddSignal(FatCatButton.Activated, function()
		if ButtonIsDragged then
			ButtonIsDragged = false
			return
		end
